...
function handleClearCurrentMonth() {
  ...
  ElMessageBox.confirm(`确定要清空${monthLabel}的所有应发数据吗？此操作不可恢复！`, '警告', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning',
  }).then(async () => {
    try {
      // 获取当前月的所有数据ID
      const res = await salaryYingFaApi.getList({
      ...
      if (res && res.data && res.data.results) {
        ...
        const deleteRes = await salaryYingFaApi.deleteChecked(ids)
